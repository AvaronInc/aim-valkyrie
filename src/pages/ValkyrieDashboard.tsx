...
interface LiveEvent { ts?: string; type?: string; src_ip?: string; risk?: number; action?: string; [k: string]: unknown; }
interface AgentData {
  agent?: string; description?: string; thinking?: string; upstream_from?: string[];
  confidence?: number; grade?: string; plan?: Array<{ type: string; reason: string }>;
      ...
      <div className="p-4 pb-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {data.campaign?.is_campaign && <Badge className="border-destructive/40 bg-destructive/10 text-destructive">CAMPAIGN</Badge>}
          </div>
        </div>
        {data.description && <p className="text-xs text-muted-foreground mt-1.5">{data.description}</p>}
      </div>
      ...
      {subagentKeys.length > 0 && (
        <div className="border-t border-border">
          <button onClick={() => setExpanded(e => !e)}
            ...
            <span className="flex items-center gap-1.5"><Cpu className="h-3 w-3" />{subagentKeys.length} Subagent{subagentKeys.length !== 1 ? 's' : ''}</span>
...
const COUNTRY_CENTERS: Record<string, { cx: number; cy: number; label: string }> = {
  US: { cx: 200, cy: 200, label: 'United States' }, CA: { cx: 200, cy: 130, label: 'Canada' },
  MX: { cx: 210, cy: 265, label: 'Mexico' },        BR: { cx: 330, cy: 350, label: 'Brazil' },
  AR: { cx: 305, cy: 415, label: 'Argentina' },     GB: { cx: 472, cy: 150, label: 'United Kingdom' },
  FR: { cx: 485, cy: 170, label: 'France' },        DE: { cx: 500, cy: 157, label: 'Germany' },
  NL: { cx: 492, cy: 152, label: 'Netherlands' },   RU: { cx: 630, cy: 130, label: 'Russia' },
  CN: { cx: 720, cy: 210, label: 'China' },         IN: { cx: 660, cy: 255, label: 'India' },
  JP: { cx: 790, cy: 195, label: 'Japan' },         KR: { cx: 770, cy: 205, label: 'South Korea' },
  AU: { cx: 770, cy: 390, label: 'Australia' },     ZA: { cx: 535, cy: 390, label: 'South Africa' },
  NG: { cx: 500, cy: 295, label: 'Nigeria' },       EG: { cx: 555, cy: 240, label: 'Egypt' },
  TR: { cx: 575, cy: 205, label: 'Turkey' },        IR: { cx: 610, cy: 220, label: 'Iran' },
  SA: { cx: 595, cy: 255, label: 'Saudi Arabia' },  VN: { cx: 730, cy: 270, label: 'Vietnam' },
  ID: { cx: 750, cy: 315, label: 'Indonesia' },     PK: { cx: 640, cy: 235, label: 'Pakistan' },
  UA: { cx: 555, cy: 168, label: 'Ukraine' },       PL: { cx: 525, cy: 157, label: 'Poland' },
  SE: { cx: 515, cy: 125, label: 'Sweden' },        ES: { cx: 470, cy: 190, label: 'Spain' },
  IT: { cx: 510, cy: 185, label: 'Italy' },         KZ: { cx: 635, cy: 180, label: 'Kazakhstan' },
};
const LAND_SHAPES = [
...
function guessCountry(ip: string): string {
  ...
  if (f <= 10)  return 'US'; if (f <= 20)  return 'CN'; if (f <= 40)  return 'RU';
  if (f <= 60)  return 'IN'; if (f <= 80)  return 'BR'; if (f <= 100) return 'DE';
  if (f <= 110) return 'US'; if (f <= 130) return 'CN'; if (f <= 140) return 'KR';
  if (f <= 150) return 'JP'; if (f <= 160) return 'US'; if (f <= 170) return 'IN';
  if (f <= 180) return 'CN'; if (f <= 190) return 'BR'; if (f <= 200) return 'AR';
  if (f <= 210) return 'US'; if (f <= 230) return 'CN'; if (f <= 240) return 'ZA';
  return 'RU';
}
function WorldThreatMap({ blockedIps }: { blockedIps: string[] }) {
  ...
  const dots = Object.entries(byCountry).map(([cc, ips]) => {
    const center = COUNTRY_CENTERS[cc];
    const ip = ips[0];
    return center
      ? { ip, x: center.cx, y: center.cy, country: cc, countryLabel: center.label }
      : { ip, x: 50 + ((ip.charCodeAt(0) * 37) % 900), y: 30 + ((ip.charCodeAt(2) * 53) % 440), country: cc, countryLabel: cc };
  });
  const extraDots = blockedIps.slice(0, 30).map((ip, i) => {
    const cc = guessCountry(ip);
    const center = COUNTRY_CENTERS[cc];
    return {
      ip, country: cc,
      x: (center ? center.cx : 50 + ((ip.charCodeAt(0) * 37) % 900)) + ((ip.charCodeAt(1) * 13 + i * 17) % 30) - 15,
      y: (center ? center.cy : 30 + ((ip.charCodeAt(2) * 53) % 440)) + ((ip.charCodeAt(3) * 11 + i * 19) % 20) - 10,
    };
  });
    ...
    <div className="space-y-4">
      <div className="rounded-lg overflow-hidden border border-border">
        <svg viewBox="0 0 1000 500" className="w-full" style={{ background: 'oklch(0.10 0.015 240)' }}>
          {Array.from({ length: 19 }).map((_, i) => <line key={`v${i}`} x1={(i*1000)/18} y1={0} x2={(i*1000)/18} y2={500} stroke="oklch(0.82 0.22 145 / 5%)" strokeWidth={0.5} />)}
          {Array.from({ length: 10 }).map((_, i) => <line key={`h${i}`} x1={0} y1={(i*500)/9} x2={1000} y2={(i*500)/9} stroke="oklch(0.82 0.22 145 / 5%)" strokeWidth={0.5} />)}
          <line x1={0} y1={250} x2={1000} y2={250} stroke="oklch(0.82 0.22 145 / 15%)" strokeWidth={0.8} strokeDasharray="4 4" />
          {LAND_SHAPES.map((d, i) => <path key={i} d={d} fill="oklch(0.22 0.02 240)" stroke="oklch(0.82 0.22 145 / 20%)" strokeWidth={0.8} />)}
          {Object.entries(COUNTRY_CENTERS).map(([cc, { cx, cy }]) => <circle key={cc} cx={cx} cy={cy} r={2} fill="oklch(0.82 0.22 145 / 25%)" />)}
          {extraDots.map((d, i) => <circle key={`e${i}`} cx={d.x} cy={d.y} r={3} fill="oklch(0.65 0.28 25 / 30%)" stroke="oklch(0.65 0.28 25 / 50%)" strokeWidth={0.5} />)}
          {dots.map(dot => (
            <g key={dot.country} className="cursor-pointer" onClick={() => setSelected(selected === dot.country ? null : dot.country)}>
              <circle cx={dot.x} cy={dot.y} r={16} fill="none" stroke={selected === dot.country ? 'oklch(0.65 0.28 25 / 60%)' : 'oklch(0.65 0.28 25 / 20%)'} strokeWidth={1} />
              <circle cx={dot.x} cy={dot.y} r={10} fill="none" stroke={selected === dot.country ? 'oklch(0.65 0.28 25 / 40%)' : 'oklch(0.65 0.28 25 / 15%)'} strokeWidth={1} />
              <circle cx={dot.x} cy={dot.y} r={6} fill={selected === dot.country ? 'oklch(0.65 0.28 25 / 80%)' : 'oklch(0.65 0.28 25 / 50%)'} stroke="oklch(0.65 0.28 25)" strokeWidth={1.5} />
              {(byCountry[dot.country]?.length ?? 0) > 1 && (
                <>
                  ...
                  <text x={dot.x+8} y={dot.y-5} textAnchor="middle" fontSize={7} fontWeight="bold" fill="oklch(0.65 0.28 25)">{byCountry[dot.country]?.length}</text>
                </>
              )}
              <text x={dot.x} y={dot.y+22} textAnchor="middle" fontSize={7} fill={selected === dot.country ? 'oklch(0.90 0.05 240)' : 'oklch(0.70 0.05 240)'}>{dot.country}</text>
            </g>
          ))}
        </svg>
      </div>
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5"><span className="inline-block w-3 h-3 rounded-full bg-destructive/50 border border-destructive" />Blocked IP origin</span>
        <span className="flex items-center gap-1.5"><span className="inline-block w-2 h-2 rounded-full" style={{ background: 'oklch(0.82 0.22 145 / 30%)' }} />Monitored region</span>
        ...
        <Card className="border-destructive/30">
          <CardHeader>
            <CardTitle className="text-foreground text-sm font-semibold flex items-center gap-2">
              <Ban className="h-4 w-4 text-destructive" />{selectedDot.countryLabel} — {selectedIps.length} blocked IP{selectedIps.length !== 1 ? 's' : ''}
            </CardTitle>
...
export default function ValkyrieDashboard() {
  // ← recentEvents now comes from the hook (checks all possible field names)
  const { payload, wsStatus, state, agents, approvalQueue, actionLog, recentEvents, defenseMode } = useValkyrie();
  ...
  const riskScore    = (state as Record<string,unknown>)?.risk as number ?? 0;
  const failedAuths  = (state as Record<string,unknown>)?.failed as number ?? 0;
  const blockedIps   = (state as Record<string,unknown>)?.blocked_ips as string[] ?? [];
  const agentKeys    = Object.keys(agents);
  const events       = recentEvents as LiveEvent[];
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-3">
            <Shield className="h-7 w-7 text-matrix" />Valkyrie
          </h1>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
        <Card><CardHeader><CardTitle>Risk Score</CardTitle><AlertTriangle className={`h-4 w-4 ${riskScore >= 80 ? 'text-destructive' : riskScore >= 50 ? 'text-warning' : 'text-matrix'}`} /></CardHeader>
          <CardContent><div className={`text-2xl font-bold ${riskScore >= 80 ? 'text-destructive' : riskScore >= 50 ? 'text-warning' : 'text-matrix'}`}>{payload ? riskScore : '–'}</div><p className="text-xs text-muted-foreground mt-1">{riskLabel(riskScore)}</p></CardContent>
        </Card>
        <Card><CardHeader><CardTitle>Failed Auths</CardTitle><XCircle className="h-4 w-4 text-destructive" /></CardHeader>
          <CardContent><div className="text-2xl font-bold text-foreground">{payload ? failedAuths : '–'}</div><p className="text-xs text-muted-foreground mt-1">{failedAuths === 0 ? 'No failures' : 'Auth failures logged'}</p></CardContent>
        </Card>
        <Card><CardHeader><CardTitle>Blocked IPs</CardTitle><Ban className="h-4 w-4 text-warning" /></CardHeader>
          <CardContent><div className="text-2xl font-bold text-foreground">{payload ? blockedIps.length : '–'}</div><p className="text-xs text-muted-foreground mt-1">{blockedIps.length === 0 ? 'No IPs blocked' : 'Active blocks'}</p></CardContent>
        </Card>
        <Card><CardHeader><CardTitle>Pending Approvals</CardTitle><Shield className="h-4 w-4 text-tech" /></CardHeader>
          <CardContent><div className={`text-2xl font-bold ${approvalQueue.length > 0 ? 'text-warning' : 'text-foreground'}`}>{payload ? approvalQueue.length : '–'}</div><p className="text-xs text-muted-foreground mt-1">{approvalQueue.length === 0 ? 'Queue clear' : 'Awaiting review'}</p></CardContent>
        </Card>
      </div>
      ...
      <div className="grid w-full grid-cols-3 gap-1 rounded-lg bg-muted p-1 sm:grid-cols-6">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            {tab.id === 'blocked' && blockedIps.length > 0 && <span className="ml-0.5 rounded-full bg-warning/20 text-warning text-[10px] px-1.5 py-0.5 border border-warning/30">{blockedIps.length}</span>}
            {tab.id === 'overview' && approvalQueue.length > 0 && <span className="ml-0.5 rounded-full bg-destructive/20 text-destructive text-[10px] px-1.5 py-0.5 border border-destructive/30">{approvalQueue.length}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
                ...
                {agentKeys.map(key => {
                  ...
                  return (
                    <div key={key} className="flex items-center justify-between p-3 rounded-lg bg-background/50 mb-2 last:mb-0">
                      <div className="flex items-center gap-3">
                        <div className={`w-3 h-3 rounded-full ${agent?.thinking ? 'bg-matrix' : 'bg-muted-foreground'}`} />
                        <div>
                      ...
                      {agent?.grade
                        ? <Badge className={gradeBadge(agent.grade)}>{agent.grade}</Badge>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-1.5 max-h-64 overflow-y-auto">
                  {events.length === 0 && <div className="py-8 text-center text-muted-foreground"><Activity className="h-10 w-10 mx-auto mb-3 opacity-40" /><p className="text-sm">Waiting for events…</p></div>}
                  {[...events].reverse().map((ev, i) => (
                    <div key={i} className="rounded-lg bg-background/50 p-2.5 text-xs font-mono">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {ev.type && <span className="text-tech shrink-0">{ev.type}</span>}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-foreground text-base font-semibold flex items-center gap-2"><Shield className="h-4 w-4 text-tech" />Approval Queue</CardTitle>
            ...
            <CardContent>
              ...
              <div className="space-y-3">
                {approvalQueue.map((item: Record<string,unknown>) => (
                  <div key={item.req_id as string} className="rounded-lg border border-border bg-background/50 p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                      <div>
                        <span className="text-sm font-bold text-destructive font-mono">{item.action as string}</span>
                        <span className="text-xs text-muted-foreground ml-2">#{item.req_id as string}</span>
                        <p className="text-xs text-warning font-mono mt-0.5">{item.ip as string}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">Agent: {(item.agent as string) ?? 'system'}</p>
                      </div>
                      <Badge className={riskBadge(item.risk as number)}>risk={item.risk as number}</Badge>
                    </div>
                    <div className="flex gap-2">
                      <Button onClick={() => decide(item.req_id as string, 'approved')} disabled={approveLoading === item.req_id} variant="default" className="flex-1">
                        {approveLoading === item.req_id ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle className="h-3 w-3 mr-1" />}Approve
                      </Button>
                      <Button onClick={() => decide(item.req_id as string, 'denied')} disabled={approveLoading === item.req_id} variant="destructive" className="flex-1">
        ...
        <Card>
          <CardHeader>
            ...
            <input className="bg-background border border-border rounded-md px-3 py-1.5 text-xs font-mono outline-none focus:border-matrix w-48"
              placeholder="Filter by IP…" value={ipFilter} onChange={e => { setIpFilter(e.target.value); setTimelinePage(1); }} />
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-xs font-mono">
                <thead><tr className="border-b border-border">
                  {['Timestamp','Source IP','Event','Risk','Action'].map(h => <th key={h} className="text-left py-2 px-3 text-muted-foreground uppercase tracking-wider text-[10px]">{h}</th>)}
                </tr></thead>
                <tbody>
                  {timelineLoading && <tr><td colSpan={5} className="py-8 text-center text-muted-foreground"><Loader2 className="h-5 w-5 animate-spin mx-auto mb-2" />Loading…</td></tr>}
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <span className="text-xs text-muted-foreground">Page {timelinePage}</span>
        ...
        <Card>
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-xs font-mono">
                <thead><tr className="border-b border-border">
                  {['Timestamp','Action','Target IP','Executed'].map(h => <th key={h} className="text-left py-2 px-3 text-muted-foreground uppercase tracking-wider text-[10px]">{h}</th>)}
                </tr></thead>
                <tbody>
                  {actionLog.length === 0 && <tr><td colSpan={4} className="py-8 text-center text-muted-foreground">No defense actions yet.</td></tr>}
                  {actionLog.map((a, i) => (
                    <tr key={i} className="border-b border-border hover:bg-accent/40 transition-colors">
                      ...
                      <td className="py-2 px-3">{a.executed ? <span className="flex items-center gap-1 text-matrix"><CheckCircle className="h-3 w-3" />Yes</span> : <span className="flex items-center gap-1 text-destructive"><XCircle className="h-3 w-3" />No</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {blockedIps.map((ip: string) => (
                <div key={ip} className="rounded-lg border border-border bg-background/50 p-4 flex flex-col gap-3">
                  <div><span className="text-sm font-bold text-destructive font-mono block">{ip}</span><span className="text-xs text-muted-foreground">Blocked by system</span></div>
      ...
      {activeTab === 'agents' && (
        <div className="space-y-4">
          {agentKeys.length === 0 && <Card><CardContent><div className="py-12 text-center text-muted-foreground"><Cpu className="h-12 w-12 mx-auto mb-4 opacity-40" /><p className="text-sm font-medium">Waiting for agent telemetry…</p></div></CardContent></Card>}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {agentKeys.map(key => <AgentCard key={key} agentKey={key} data={agents[key] as AgentData} />)}
          </div>
        </div>
      )}
      ...
      {activeTab === 'map' && (
        <Card>
          <CardHeader>
            <CardTitle className="text-foreground text-base font-semibold flex items-center gap-2"><Map className="h-4 w-4 text-tech" />World Threat Map</CardTitle>
            <span className="text-xs text-muted-foreground">{blockedIps.length} IPs plotted — click a dot to expand</span>
          </CardHeader>
          <CardContent><WorldThreatMap blockedIps={blockedIps} /></CardContent>
        </Card>
      )}
    </div>
  );
